...
| **Localization (i18n)** | `core/i18n.py` + catalogs `core/locales/en.json`, `core/locales/ar.json`. Startup validation (fail-fast): non-default catalogs must be a key-subset of the default AND a placeholder-subset per key (an alien `{placeholder}` would surface as a raw template — V2-D-026). Per-locale user-facing coverage (admin/owner namespaces carved out) is computed at `configure()` and published to the `i18n_catalog_coverage{locale}` gauge by each composition root |
| **Feature flags (V2)** | `services/feature_flag_service.py` — every major V2 feature behind an individually toggleable `bool` row in the LOCKED `settings` table (V2-D-031, plan §5.8), read through the cached `SettingsService` (off by default; a missing row reads off), toggled with audit logging. Rows seeded off by migration `2026072001_seed_feature_flags`. Flags: `feature_subscriptions_enabled`, `feature_multilink_enabled`, `feature_premium_quality_gating_enabled`, `feature_ad_analytics_enabled`, `feature_expiry_notifications_enabled`, `analyzer_raw_metadata_enabled` |
| **Subscriptions & entitlements (V2.1, SHADOW)** | Design: `DESIGN_SUBSCRIPTIONS.md`. Tables `plans` (JSONB `entitlements`) + `subscriptions` (one active row per user via partial unique index) — migrations `2026072002`/`2026072003`. Registry `domain/entitlements.py` (validated at boot — a malformed plan fails startup). Read path `services/entitlement_service.py` (`PlanCatalog` + `EntitlementService.resolve`, lazy expiry); write path `services/subscription_service.py` (grant/extend/remove/set_expiration, audit-logged). **Shadow mode:** the resolver runs at snapshot-fill via `services/entitlement_shadow.py` and only feeds `entitlement_parity_mismatch_total` — `is_premium` stays authoritative until the V2.2 cutover. Backfill: `scripts/backfill_subscriptions.py` (dry-run default). Entities `domain/entities/plan.py`·`subscription.py`; enums `domain/enums/subscription.py`; repos `infrastructure/database/repositories/plan.py`·`subscription.py` |
