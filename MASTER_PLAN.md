...
| V1 Sprint 9.5.10 | `202606250002_scheduling` | **Applied** (head `202606250002`). Adds nullable `broadcasts.scheduled_at` + `advertisements.scheduled_at` and a partial index `ix_broadcasts_scheduled` (pending + scheduled rows). Additive-only; NULL preserves immediate/always-eligible behavior. D-053. |
