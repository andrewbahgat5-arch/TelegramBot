...
| D-075 | 2026-07-06 | **Reward Engine — generalise the referral reward (refactors D-066).** Replace the hard-coded `users.referral_bonus_downloads` counter with a generic, open-for-extension reward model. **Domain** (`domain/rewards.py`): a `RewardType` enum (`daily_download_bonus`, `premium_days`, `download_credits`, `queue_priority`, `ad_free`, `unlock_quality`), a `StackingRule` enum (`SUM`/`MAX`/`ANY`, each with `aggregate()`), and a `REWARD_DEFINITIONS` registry mapping each type to its stacking rule — the **single OCP seam** (a new type = one enum member + one registry entry). **Entity** `Reward` (`value`, optional `param`, `source`, nullable `expires_at`; `is_active(now)`, null expiry = permanent). **Persistence**: a `rewards` table + `RewardRepository` (active lookups filter expiry in SQL). **Service** `RewardService.grant()/active_value(user,type)/active_rewards()` — `active_value` filters expired rewards and applies the type's stacking rule. **Consumption**: `RateLimitService` reads `active_value(user, DAILY_DOWNLOAD_BONUS)` through a narrow `RewardReader` protocol (effective_limit = base + active bonus) — it names an *effect*, never a source. A **referral grants** a permanent `DAILY_DOWNLOAD_BONUS` reward (existing D-066 behavior preserved as a configured reward); the `referral_bonus_downloads` column is dropped (migration `202607060001` backfills it into permanent rewards first). Reserved: D-070–D-074 are the (unmerged, planned-elsewhere) §4.4 Platform Compatibility Rules — skipped here to avoid a future renumber. | The Owner no longer wants "a referral reward is always +daily downloads". A generic engine lets a referral (or any future source) grant *any* reward type — premium days, credits, queue priority, ad-free, quality unlocks — without editing the download-limit logic or any other consumer (Open/Closed): a consumer depends on an effect + the engine, never on a reward source. Per-type stacking rules and nullable expiry cover the "stack according to configurable rules" and "support expiration" requirements with three small enums rather than a rules DSL (kept deliberately simple). Filtering expiry in SQL keeps the per-download hot path to one indexed query. | Keeping `referral_bonus_downloads` and special-casing each new reward in the rate limiter (violates OCP — every reward type would touch the download-limit code); a fully generic effect-resolver/plugin system with per-type consumer registration (over-engineered — only the daily-download effect has a consumer today); storing rewards in the flat `settings` table (no per-user scoping, expiry, or stacking); a JSON `rewards` blob on `users` (not queryable/indexable for active-reward lookups or future admin reporting). |
