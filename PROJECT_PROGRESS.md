...
| Last code change | 2026-06-24 — Owner verification feedback fixes #14–#20 (download rate-limit enforcement + cache invalidation; free single-active-job cap; broadcast excludes Owner/Moderator; silent-ignore admin commands; `/users`) + the Sprint-7 `history_page_size` carry-over fix. |
| Last documentation change | 2026-06-24 — feedback-round (#14–#20) handoff (this entry); TEST_RESULTS updated. |
| Next recommended action | **Re-verify the feedback fixes live** (free-limit blocks after N, one active download at a time, broadcast skips Owner, admin commands hidden from normal users, `/users`) and **run the integration suite with Docker up** to confirm the #14 audience SQL. Then decide task 8.3 (FastAPI) and authorize Sprint 9. |
