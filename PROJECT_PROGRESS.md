...
|---|---|---|---|---|
| 2026-07-06 | Reward Engine refactor (referral → generic rewards, D-075) | Unit + Security = **862** + ruff + format + mypy --strict + import-linter + bandit | PASS | +24 tests (rewards domain, RewardService, referral/rate-limit rewritten to rewards). **mypy --strict 0 / 179**; **import-linter 7/7** (the new `RewardReader`/`RewardEngine` consumer protocols keep `services` decoupled from a concrete engine); ruff+format clean (328 files); **bandit 0**. Single Alembic head → **202607060001** (creates `rewards`, backfills `referral_bonus_downloads` → permanent daily-download rewards, drops the column). No download-limit or referral *behavior* change — existing +daily-download bonus is preserved as a configured `DAILY_DOWNLOAD_BONUS` reward. Integration/e2e not runnable here (no live services). |
| 2026-07-06 | Railway deployment readiness (worker yt-dlp fix, api PORT, docs/scripts) | Unit (793) + Security (45) = **838** + ruff + format + mypy --strict + import-linter + bandit | PASS | +8 tests (`test_deploy_dockerfiles.py` ×3, `resolve_bind_port` ×5). ruff + format clean (321 files); **mypy --strict 0 / 174**; **import-linter 7/7**; **bandit 0**. Two container/deploy blockers fixed: worker image now installs pinned `yt-dlp==2026.7.4` (was invoked as a binary but never installed → every download would fail); api now binds Railway's dynamic `PORT` (fallback `API_BIND_PORT`). No business logic changed. Integration/e2e still not runnable here (no live services). |
| 2026-07-06 | V1-closeout full run (Sprint 12 A2–A6, Sprint 13.2 residual UI, Owner req #11 fix) | Unit (785/785) + Security (45/45) + ruff + mypy --strict + import-linter + bandit + pip-audit | PASS | **Unit 785 pass** (was 779 pre-#11; +6 rate-limit cases). **Security suite 45/45.** ruff check + format clean (320 files); **mypy --strict 0 issues / 174 files**; **import-linter 7/7**; **pip-audit 0 CVEs**. **bandit: fixed 1 pre-existing HIGH** — B613 trojan-source flagged the intentional U+200F RTL mark in `bot/panel/ui.py` (Sprint 13.1; never bandit-scanned in a recorded clean run since it was added); suppressed with a justified `# nosec B613`, now **0 all severities**. Also fixed a test regression from A2: `test_no_real_env_committed` allowlist extended to `.env.production.example`. **Integration + e2e NOT run** — no live Postgres/Redis/pgbouncer or Docker in this session (ports closed); environment constraint, not a regression (same suites passed live in prior sessions, e.g. Sprint 9 "live pg:15 + redis:7"). Owner req #11: found + fixed the D-066 referral-bonus enforcement gap (bonus was granted/shown but not added to the effective daily limit); test-first per D-031. |
...
| Field | Value |
|---|---|
| **Session goal** | Split ads into two independently-configurable layers with **minimal architecture change, no schema change, max reuse** (owner spec). Test-first; **not pushed**. Worktree `happy-bose-71ed46`. Continues the same uncommitted work as the entry below. |
| **Key realisation** | The existing standalone-ad path (`AdService.maybe_show` at `post_download`) **already is** the "Follow-up Message" layer (full media + buttons + targeting + LRS + analytics) — so it was only relabelled, not rewritten. The **only new capability is the Caption layer**. |
| **Caption layer (new)** | New `AdPlacement.CAPTION` (a string value — **no migration**; gate defaults ON alongside `post_download`). `AdService.select_caption_ad(...)` reuses the extracted `_select_due_ad` (audience + `scheduled_at` + every-N + priority/LRS) but **returns text + buttons instead of sending**, recording the impression + advancing `last_shown_at`. New **`services/caption_ad_mixer.py`** (`CaptionAdMixer.decorate/decorate_for_user`) is the single injection point: appends the ad text to a media caption (≤1024, best-effort, never raises) + returns its `AdButtonSpec`s. |
| **Transport** | `FileSenderProtocol.upload/send_cached` gained an optional `buttons: Sequence[AdButtonSpec] = ()` (existing callers unchanged); `TelegramFileSender` builds the keyboard via `build_ad_keyboard` (renamed from `_build_markup` in `ad_sender.py`) and passes `reply_markup` to send_video/audio/document. |
| **Pipeline (every media send)** | The mixer is injected into `DownloadService` (fresh upload + waiter fan-out), `JobService` (cache hit), `HistoryService` (resend), and the **analysis/"Choose a format" screen** (`bot/handlers/download.py` — ad text under the caption + CTA appended via `bot/keyboards/format_select.append_ad_buttons`). Composed in `bot/main.py` (`make_caption_mixer`) + `workers/main.py`. No per-handler duplication. |
| **Admin UI** | Placement step relabels `post_download` → **"Follow-up message"**, adds **"In media caption"**; a clear text+buttons-only limitation warning shows on the Placement screen when caption is selected. Caption ads with no text are simply inert (selector guards). |
| **Future-compat (design only)** | premium-only / free / per-language caption ads already work through the shared audience `ctx`; delayed/random/A-B/multi-campaign sit behind the `_select_due_ad` seam + `ad_events`. Not implemented. |
| **Two local-test fixes (this session, separate commits)** | **(1)** `build_bot` set no aiohttp timeout → aiogram's **60 s** default made a ~20 MB 1080p upload time out (exactly 60 s in logs) and retry-loop-block the user; now the session timeout = `WORKER_JOB_TIMEOUT` (300 s). **(2)** `#12` handler now catches transient/`InfrastructureError` → a "busy, try again" message (this is how the missing-`yt-dlp` binary surfaced cleanly during local testing). |
| **Gates** | **Full `tests/unit` green** (new `test_caption_ad_mixer.py` + `select_caption_ad` tests + a delivery-routing test proving the ad rides on the media); **mypy --strict 0/174**; ruff clean on all touched files (2 pre-existing `bot/panel/ui.py` F841s are uncommitted prior-session WIP, not from this work). Integration/e2e not run in-session (no live PG/Redis). **No migration added.** |
| **Files (primary)** | `domain/enums/ad_placement.py`, `services/ad_service.py`, `services/caption_ad_mixer.py` (new), `domain/protocols/file_sender.py`, `infrastructure/telegram/{file_sender,ad_sender}.py`, `services/{download_service,job_service,history_service}.py`, `bot/handlers/{download,history}.py`, `bot/keyboards/format_select.py`, `bot/panel/registry.py`, `bot/handlers/admin_wizard.py`, `bot/main.py`, `workers/main.py`, `infrastructure/telegram/client.py`, `core/locales/{en,ar}.json` + matching `tests/unit/*`. |

### Session Handoff — 2026-07-07 — Admin Panel UX/logic sprint: Broadcast + Advertisement wizards, audience, scheduling, link reliability

| Field | Value |
|---|---|
| **Session goal** | A logic / UX / consistency / reliability pass (not a feature sprint) over the Admin Panel's **Broadcast** and **Advertisement** flows, plus the root cause of intermittent link failures. Test-first; **not pushed**, awaiting Owner review. Worktree `happy-bose-71ed46`. |
| **#1/#2 Redundant "Type" step removed** | The compose wizard no longer asks "Broadcast or Advertisement" — the kind is fixed by the entry section. `STEP_TYPE` + `build_wizard_type` deleted; the wizard opens on **Audience**. Broadcast-menu shortcuts (Free/Premium/All/By-language) now **pre-seed** the audience (`admin_wizard.start(preset=…)`). |
| **#3 Audience consistency** | Selecting both **Free + Premium** (which OR to "everyone" in SQL) now **collapses to All** (`_collapse_all_users`), so targeting is never duplicated. Ad and broadcast share the identical audience engine (#8) — no behavioural fork. |
| **#4 Audience UI** | Kept the compact 2-per-row grid (Owner choice); audience summary in the message body localised + shows the full selection. |
| **#5 Cancel fixed (real bug)** | `cx` is a **read** action, so the wizard's Cancel was landing in the panel's read handler, whose blanket `state.clear()` wiped the wizard and rendered nothing → Cancel looked dead. Added `admin_wizard.cancel()` and routed `w/cx` to it **before** the read handler clears state; resilient when state was already lost. |
| **#6 Wizard no longer expires** | Root cause: `Dispatcher()` had **no FSM storage** → in-memory, so any restart / second replica dropped in-progress wizards. Now `Dispatcher(storage=RedisStorage(redis=cache))` (no TTL) — wizard state survives restarts and is shared across replicas. |
| **#7 Recipient estimate** | Broadcast Preview shows `Estimated recipients: N` (count only, no names) via new `BroadcastService.estimate_recipients` (pure count, **persists nothing**). Ads show no single reach number (they deliver over time). |
| **#9 Post-download conflict warning** | Enabling a second active Post-download ad (Enable button **or** wizard Save) warns and offers **Keep both** (default) / **Replace existing** / **Cancel**, naming both ads. New `AdService.active_conflicts` / `targets_placement` / `replace_active_on_placement`. |
| **#10 Deterministic fair scheduling** | Exactly one ad per download; among equal-priority **due** ads the **least-recently-shown** wins (priority tier + LRS). New nullable `advertisements.last_shown_at` (migration **`202607070001`**, new single head) + index; ordering `priority DESC, last_shown_at ASC NULLS FIRST, id ASC` **is** the swappable strategy; stamped in the impression write. Documented in new **`ADS_SCHEDULING.md`** (with a worked A/B/C example). Chose LRS over a rotation cursor (self-corrects when ads change). |
| **#11 UX polish** | Broadcast flow trimmed to Audience → Content → Preview (the **Settings** step — Enabled/Priority — is ad-only; those controls were dead for a one-shot broadcast). Preview omits ad-only lines for broadcasts. |
| **#12 Link reliability (root cause, separate commits)** | `ytdlp_provider` misclassified transient failures as permanent: YouTube's bot-check was "content", HTTP **429** wasn't transient, and unknown exits stopped retry — and yt-dlp **stderr was never logged**. Now: bot-check/429/rate-limit/"unable to extract"/nsig/5xx/timeouts are **transient**; genuine content (private/removed/geo/members-only) stays permanent; unknown stays permanent (safe) — a deliberate, documented deviation from the plan's "make unknown transient" (the explicit markers already cover the real intermittent cases, and this avoids retrying truly-broken links). Added **in-provider retry** (2×, backoff) for extraction, **stderr logging** on every non-zero exit, and a distinct user message (`download.temporary_error`, "busy — try again") for transient failures instead of "private or removed". |
| **Tests (#13)** | All new/changed logic covered: wizard flow (no Type step, preset seeding), Free+Premium collapse, every Cancel path, Redis FSM wiring, recipient estimate (no persistence), LRS rotation + priority precedence, conflict detect/keep/replace (Enable path **and** wizard Save), ytdlp reclassification + retry + transient handler UX. |
| **Gates (this session)** | **Unit suite green** (`tests/unit`, incl. ~30 new tests); **mypy --strict 0 issues / 168 source files**; catalog parity held (added keys to both `en.json` + `ar.json`). Integration/e2e not run in-session (no live Postgres/Redis) — **the `202607070001` migration must be applied and `tests/integration/test_ad_repository.py` run in the verification phase.** |
| **Files (primary)** | `bot/panel/wizard.py`, `bot/handlers/admin_wizard.py`, `bot/handlers/admin_panel.py`, `bot/keyboards/admin_panel.py`, `bot/main.py`, `services/ad_service.py`, `services/broadcast_service.py`, `infrastructure/database/models/advertisement.py`, `infrastructure/database/repositories/advertisement.py`, `migrations/versions/202607070001_ad_last_shown_at.py`, `infrastructure/downloader/providers/ytdlp_provider.py`, `bot/handlers/download.py`, `core/locales/{en,ar}.json`, `ADS_SCHEDULING.md`, and the matching `tests/unit/*`. |
| **Not done / for review** | Not committed or pushed (awaiting Owner review, per instruction). Suggested commit grouping is in the approved plan (`~/.claude/plans/hidden-wondering-leaf.md`): A wizard flow, B audience, C cancel, D Redis FSM, E estimate, F conflict, G scheduling, H polish, then **separate** #12 downloader commit(s), then docs. |

### Session Handoff — 2026-07-06 — Reward Engine: referral reward generalised (D-075)

| Field | Value |
|---|---|
| **Session goal** | Refactor the referral system so a referral grants **generic Rewards** instead of always "+daily downloads" — an open-for-extension Reward Engine. Test-first; keep it simple; not pushed, awaiting review. |
| **What changed (architecture)** | New **Reward Engine** (decision **D-075**): `domain/rewards.py` (`RewardType` enum, `StackingRule` SUM/MAX/ANY, `REWARD_DEFINITIONS` registry — the single OCP seam), `domain/entities/reward.py` (`Reward` + `is_active`, null expiry = permanent), `rewards` table + `RewardRepository`, `services/reward_service.py` (`grant` / `active_value(user,type)` filtering expiry + applying the stacking rule / `active_rewards`). |
| **Requirements ↔ implementation** | (1)/(3) The download limit no longer references `referral_bonus_downloads` — `RateLimitService` consumes `active_value(user, DAILY_DOWNLOAD_BONUS)` via a narrow `RewardReader` protocol. (2)/(5) Reward *types* (daily-download, premium-days, credits, queue-priority, ad-free, unlock-quality) are declared with stacking rules; adding one is an enum member + a registry entry — **no engine or consumer changes** (only `DAILY_DOWNLOAD_BONUS` has a consumer today, by design). (4) Existing behavior preserved: a referral grants a permanent `DAILY_DOWNLOAD_BONUS` reward (`ReferralService` via a `RewardEngine` protocol). (6) Expiry: nullable `expires_at`, filtered in SQL + `is_active`. (7) Stacking: per-type `StackingRule` (SUM/MAX/ANY). (8) Kept simple — three small enums + one table, no rules DSL / plugin registry. |
| **Data / migration** | Migration **`202607060001`** (new single head): create `rewards`, **backfill** each `referral_bonus_downloads > 0` into a permanent `daily_download_bonus` reward (`source='referral_legacy'`), then **drop** `users.referral_bonus_downloads`. Downgrade rebuilds the column from active rewards. `User` model + `FakeUser` + `UserRepository.add_referral_bonus` + its protocol method removed; `ReferralRepository.leaderboard` now returns the referrer's `users.id` (reward-agnostic) and the service fills `rewards_earned` from the engine. |
| **Behavior** | **No user-facing behavior change.** The +daily-download referral bonus works exactly as before (now as a configured reward). Stats/`/referral`/leaderboard read the active bonus back from the engine. |
| **Gates** | Unit + security **862 pass** (+24: rewards domain, RewardService, rewritten referral/rate-limit). ruff+format clean (328 files); **mypy --strict 0/179**; **import-linter 7/7**; **bandit 0**. Integration/e2e not runnable in-session (no live Postgres/Redis). |
| **Extending it later (for the Owner)** | To make a referral grant something else (or add an admin "grant reward" action): call `RewardService.grant(user_id, RewardType.X, value=…, source=…, expires_at=…)`. To add a new reward *type*: add a `RewardType` member + a `REWARD_DEFINITIONS` entry (pick a stacking rule), then have that subsystem read `active_value(user, RewardType.X)` — the engine and all existing consumers are untouched. Decision numbers **D-070–D-074** were skipped (reserved for the unmerged §4.4 Platform Compatibility Rules) — see the prior handoff's doc-discrepancy note. |

### Session Handoff — 2026-07-06 — Railway deployment readiness (blocker fixes + deploy/validation/stress docs + scripts)

| Field | Value |
|---|---|
| **Session goal** | Prepare the repo for a first **Railway** deployment + production validation. No new features, no business-logic changes, test-first. Not pushed, not deployed — awaiting Owner review. |
| **Two real deploy blockers fixed** | **(1) yt-dlp missing from the worker image.** `YtDlpProvider` shells out to the `yt-dlp` **binary** (no Python import), but `Dockerfile.worker` only installed ffmpeg — so *any* container deploy (Railway or compose) would fail every download with "yt-dlp not found". Now installs pinned `yt-dlp==2026.7.4` (bump per OQ-9). **(2) api didn't bind Railway's PORT.** Railway injects a dynamic `PORT` a web service must listen on; the api bound only `API_BIND_PORT`. Added a tested `resolve_bind_port()` (PORT wins, fallback `API_BIND_PORT`; backward compatible). Both test-first: `test_deploy_dockerfiles.py` (guards ffmpeg + pinned yt-dlp + entrypoints) and 5 `resolve_bind_port` cases. |
| **New production scripts** | `deploy/migrate.sh` (`alembic upgrade head` — use as the Railway pre-deploy command; alembic/env builds the URL from `Settings`). `deploy/monitor-resources.sh` (samples `/v1/metrics` gauges + host RAM + temp-dir disk/size to CSV during a **real** stress run — no synthetic load; guarded probes). Both `sh -n` + dry-run verified. |
| **New docs** | `deploy/RAILWAY_DEPLOYMENT.md` (3 app services + managed PG/Redis; env-var mapping via Railway reference vars; secrets; migrations; `/v1/health` + domain; long-polling default / webhook advanced; disk/connection sizing). `deploy/PRODUCTION_VALIDATION.md` (end-to-end: startup, transport, registration, download, queue, cache, admin, referral incl. D-066 bonus, broadcast, migrations, error logging). `deploy/LARGE_DOWNLOAD_STRESS_TEST.md` (observation-based plan for all 11 requested dimensions). Runbook `deploy/README.md` companion list updated. |
| **⚠️ "50 GB" reality check** | The brief asked to stress-test files "up to ~50 GB". **The Telegram Bot API cannot upload 50 GB.** Real ceilings (code-backed): **50 MB** public API, **2 GB** self-hosted bot-api (D-040); the app's `max_file_size` defaults to **2 GiB** and rejects larger files (`FileTooLargeError`, permanent). The stress plan targets **up to 2 GB** and says so up front. |
...
| Field | Value |
|---|---|
| **Session goal** | Finish V1 completely — close every remaining V1 item doable from a coding session; produce an exact handoff for the Owner-only remainder. |
| **Worktree** | `happy-bose-71ed46` (branch `claude/happy-bose-71ed46`), the live worktree. Session was launched in a different fresh worktree; redirected here before any code, per the standing convention. |
| **Shipped (12 commits, not pushed)** | **Sprint 12 Phase A A2–A6:** `.env.production.example` (+ `.gitignore` exception + build/parity/no-secrets gate tests); `deploy/smoke-test.sh` + `deploy/SMOKE_TEST.md`; `deploy/RELEASE_CHECKLIST.md`; `.github/workflows/release.yml` (tag-triggered gates + prod-compose validation + image build; publish/deploy Owner-infra-gated); runbook drift fix (Alembic head → `202607050003`, §3 uses prod compose, admin-API line → D-051). **Sprint 13.2 residual UI:** user lists + banned/health lists, errors, jobs, ads (list/detail `ui.card`/picker/totals), grouped Settings, Broadcast — all now through `bot/panel/ui.py`; catalog parity held 464/464 en/ar. **Owner req #11:** verified D-012 lazy reset + **found & fixed the D-066 referral-bonus enforcement gap** (see below) with 7 new tests + a live manual-verification script. Plus two full-run fixes (bandit B613 nosec; security env-allowlist). |
| **Gates (this session)** | Unit **785/785**; Security suite **45/45**; ruff check+format clean (320 files); mypy --strict **0 / 174 files**; import-linter **7/7**; bandit **0 all severities**; pip-audit **0 CVEs**. **Integration + e2e not run** — no live Postgres/Redis/pgbouncer/Docker in this session (ports closed); environment constraint, not a regression (these passed live in prior sessions). |
| **⚠️ Behavior change to review (D-066)** | Owner req #11 verification surfaced that Sprint 13.7 granted + displayed `referral_bonus_downloads` but **never applied it to enforcement**. D-066 / `SPRINT_13_PLAN` §13.7 specify `effective_limit = base_daily_limit + referral_bonus_downloads` (permanent, never reset by D-012). `RateLimitService.check_download` now adds the bonus. This is a real user-facing change (referred users can now actually download more) — flagged for Owner awareness; it matches the documented design, not a new product decision. |
| **📄 Doc discrepancy flagged** | The session brief referenced a **MASTER_PLAN §4.4 "Platform Compatibility Rules" (D-070–D-074)** as binding, and a `V3_PLAN_BOT_FACTORY.md`. **Neither exists in any branch** (confirmed via `git grep` across branches). All work this session nonetheless respects their evident spirit (already embodied in `ui.py`: HTML-escaping, RTL-awareness, Fragment-gated animated emoji). If §4.4/D-070–D-074 are meant to be canonical, they still need to be authored into MASTER_PLAN. |

#### ▶ V1 COMPLETION — OWNER CHECKLIST (everything that remains, all Owner-only)

Nothing further is doable from a coding session. The four groups below are the entire remaining path to a shipped, signed-off V1.

**1. Pending sign-offs + the push decision (do these first — they gate the rest)**
- [ ] **Sign off Sprint 9** (Smart Advertisements), **Sprint 9.5** (Ads v2), **Sprint 10** (Observability & Backup), **Sprint 13** (Admin Panel Enhancement) — all are `[~]` code-complete/Under-Review. Review the diffs (and ideally a live-bot walkthrough) and record sign-off here.
- [ ] **Decide the push** for `claude/happy-bose-71ed46` (this session added 12 commits on top of the 21 already ahead of origin; nothing is pushed per the standing rule). Push when ready: `git -C .claude/worktrees/happy-bose-71ed46 push origin claude/happy-bose-71ed46`.
- [ ] **Review the D-066 referral-bonus behavior change** above and confirm it's intended (it is per D-066); no code action needed if so.

**2. Sprint 11 Phase B — live test infra + runs (Owner-provisioned sandbox)**
- [ ] Create a **@BotFather sandbox bot** (separate from production); capture its token.
- [ ] Stand up **isolated infra**: a test PostgreSQL (`alembic upgrade head` → `202607050003`), isolated Redis (separate logical DBs + `test:` queue prefix), isolated storage (`/tmp/test_downloads`), and a dedicated test-owner account.
- [ ] Set **`DEPLOY_ENV=test`** and **`PROD_BOT_TOKEN_FINGERPRINT`** = SHA-256 of the prod token (arms the D-032/D-060 boot guard), and **`E2E_LIVE=1`**.
- [ ] Implement the two remaining live-transport stubs (`tests/simulation/bot_client/client.py::SandboxBotClient`, `tests/e2e/harness.py::E2EHarness.connect`) — deferred here because they're untestable without the sandbox.
- [ ] **Execute the live runs 11.10–11.14**: L1–L4 load, ST-1…ST-6 (incl. fault injection), S-1…S-5 scenarios, full regression + every security category live, the final capacity report, and record M-17…M-20. These runs **also carry** the parked **Sprint 11.5** (Arabic RTL / language fan-out) and **Sprint 13** (dashboard/referral/templates) **live-bot walkthroughs**.
- [ ] **Mark Sprint 11 complete** once the Exit Criteria are met and you sign off the capacity report.

**3. Sprint 12 Phase B — production deploy (tasks 12.1–12.7)**
- [ ] Resolve the launch-gating open questions: **OQ-1** (Sentry: self-hosted vs SaaS + per-env DSN), **OQ-3** (production host), **OQ-5** (Telegram alerts chat id). Also OQ-11 (50 MB vs self-hosted 2 GB Bot API) if not already settled.
- [ ] Provision production infra; fill `.env.production` from `.env.production.example` (secrets from the secret backend).
- [ ] Run **`deploy/RELEASE_CHECKLIST.md`** end-to-end: gates → build/tag → migrate → deploy → **`deploy/smoke-test.sh` + `deploy/SMOKE_TEST.md`** → forced-alert test → re-run E2E S-1/S-2 in prod → append release entries to `TEST_RESULTS.md`, `SECURITY_REPORT.md`, `PERFORMANCE_REPORT.md` → **mark V1 complete** → schedule the T+30 restore drill and T+90 L4 shadow-load run → **Gate G-8** sign-off. (12.1–12.7.)
- [ ] Also run **`DAILY_LIMIT_VERIFICATION.md`** live (Owner req #11 manual half: daily-reset boundary + referral-bonus stacking/permanence).

**4. Optional / non-blocking**
- [ ] **Activate animated panel emoji (D-067)** once the bot has a **Fragment-purchased username**: obtain the `custom_emoji_id`s and fill `bot/panel/ui.py::CUSTOM_EMOJI_IDS`. The whole panel switches to animated icons with **zero screen changes** — it ships as plain Unicode today by design, not a bug.
- [ ] If §4.4/D-070–D-074 are meant to be canonical platform rules, author them into `MASTER_PLAN.md` (see the doc-discrepancy note above).
