...
| V2-D-036 | **Plural strategy (recorded in Sprint V2.0, implementation deferred).** When a plural-sensitive string first lands, plurals are expressed as CLDR plural-category **suffix keys** on the existing flat catalog — `key.one`, `key.other` (and `key.few`/`key.many`/`key.zero`/`key.two` where a language needs them) — with selection via `babel`'s `Locale.plural_form`. No catalog restructuring: the keys stay flat dot-names, the loader and its startup validation (subset + placeholder checks) are unchanged, and a language that omits an optional category simply falls back to `.other` then to the default locale (existing fallback). English/Arabic ship **no** plural keys yet, so V2.0 adds the decision and the `babel` selection helper is written only when the first plural-heavy string appears. | Deciding the mechanism at 2 catalogs is a code change; retrofitting it across 20 catalogs once plural strings already exist is a data-cleanup project (V2-D-026). Suffix-key + CLDR is the industry-standard approach `babel` already implements — no bespoke plural engine. | ICU `MessageFormat` (a second templating grammar alongside `str.format`, far larger surface); ad-hoc `if n == 1` branches at call sites (scatters locale rules into Python, breaks for languages with >2 categories). |
